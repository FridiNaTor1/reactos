    ...
    _Inout_ PX86_BIOS_REGISTERS Registers)
{
    ...
    InstructionPointer[2] = 0xD6; // undefined, used as return indicator
    ...
    while (TRUE)
    {
        ...
        /* Read the next instruction and check if it's the return indicator */
        InstructionPointer = x86BiosMemoryMapping + FlatIp;
        if (*InstructionPointer == 0xD6)
        {
            /* We are done! */
            break;
        }
